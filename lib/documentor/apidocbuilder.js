...
function ApidocBuilder(globalConfig, groupConfig ){
	...
	this.build = function()
	{
		var configManager = new ConfigManager(globalConfig, groupConfig, new WorkingDirectory());
...
function ApidocManager(cfgManager){
	...
	this.getApidocCMD = function()
	{
		...
		var cmd =   util.format(apidocCMD,
			...
			configManager.getRegexToDocument(),
			configManager.getSourceDirFormat(),
	...
	this.getApidocJSON = function(){
			return {
				src:	configManager.getSourceDirFormat(),
				dest: configManager.getOutputDir(),
				options: {
					 debug: true,
					 includeFilters: configManager.getRegexToDocument(),
	...
	this.getRegexToWatch = function(){
		...
		{
			var regex = dirs[i] + "%s";
			var globfiles = configManager.getRegex(regex, configManager.getGroupK(ConfigManager.I_REGEX));
